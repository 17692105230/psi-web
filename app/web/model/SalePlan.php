...
use app\common\model\web\SalePlan as SalePlanModel;
use app\web\model\SalePlanDetails as SalePlanDetailsModel;
use app\web\model\SaleOrders as SaleOrdersModel;
use app\common\utils\BuildCode;
use think\facade\Db;
use think\Exception;

class SalePlan extends SalePlanModel
{
    public function loadData($where, $page, $rows)
    {
        $query = $this->alias('sp')
            ->join('hr_client_base cb', 'sp.client_id = cb.client_id', 'left')
            ->where(['sp.com_id' => $where['com_id']])
            // 不展示草稿
            ->where('sp.orders_status', '<>' , 7)
            //单据状态
            ...
            ->when(isset($where['search_orders_code']) && $where['search_orders_code'], function ($query) use ($where) {
                $query->where('sp.orders_code', 'like', '%' . $where['search_orders_code'] . '%');
            })
            // 单据开始时间
            ->when(isset($where['search_date_begin']) && $where['search_date_begin'], function ($query) use ($where) {
                $query->where('sp.orders_date', '>', strtotime($where['search_date_begin']));
            })
            // 单据结束时间
            ->when(isset($where['search_date_end']) && $where['search_date_end'], function ($query) use ($where) {
                $query->where('sp.orders_date', '<=', strtotime($where['search_date_end'] . ' +1 day'));
            });
        $total = $query->count();
        $offset = ($page - 1) * $rows;
        $rows = $query->limit($offset, $rows)->field('sp.*,cb.client_name')->order('sp.orders_date desc')->select();
        return compact('total', 'rows');
    }

    public function getPlanDetails($where)
    {
        return $this->alias('sp')
            ->with('details')
            ->join('hr_client_base cb','sp.client_id=cb.client_id','left')
            ->where('sp.com_id', $where['com_id'])
            ->where('sp.orders_code', $where['orders_code'])
            ->field('sp.*,cb.client_name')
            ->find();
    }

    public function delSalePlan($where)
    {
        $data = $this->getPlanDetails($where);
        if (empty($data)) {
            $this->setError('销售订单不存在');
            return false;
        }
        if ($data['orders_status'] != 0) {
            $this->setError('销售订单状态不是草稿状态不能删除');
            return false;
        }
    ...
    public function updatePlane($data)
    {
        ...
        if (!isset($data['orders_code']) || empty($data['orders_code'])) {
            $data['orders_code'] = BuildCode::dateCode('SP');
            $info = $this;
        } else {
        ...
        try {
            ...
            $info->saveData($data);
            $orders_code = $data['orders_code'];
            $detailModel = new SalePlanDetailsModel();
            //删除detail
            ...
            if($data['orders_status'] == 9){
                ...
                //获取detail
                $sale_plan_details = new SalePlanDetails();
                $transfer_data_details = $sale_plan_details->getList(['com_id' => $data['com_id'],'orders_code' => $data['orders_code']])->toArray();
                $transfer_data = $data;
                //状态改为销售单的订单状态
                $transfer_data['orders_status'] = 8;
                $transfer_data['orders_code'] = BuildCode::dateCode('SO');
                $res = $sale_order->saveTransferData($transfer_data,$transfer_data_details);
                if(!$res){
                    throw new Exception('流转至销售单失败');
                }
